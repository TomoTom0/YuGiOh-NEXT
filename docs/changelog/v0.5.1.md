...
CardList.vue の setup() 関数から不要な props をエクスポーズしていたため、検索結果が UI に反映されなかった問題を修正：

**問題**:
- 検索実行時に API から 18 件のデータが取得できているのに、UI では 0 件と表示される
- 原因：props をそのまま return していたため、template との同期が失敗

**修正**:
- `cards`, `sectionType`, `sortOrder`, `viewMode`, `uniqueId`, `showCollapseButton`, `showCodeSort` を return から削除
- Vue の template は props に直接アクセス可能なため、わざわざ return する必要がない

### キーボードショートカット（/ と Ctrl+J）が機能しない問題を修正

Pinia の `reactive()` で配列が `Proxy(Object)` に変換されるため、ショートカット マッチング処理が失敗していた問題を修正：

**問題**:
- ショートカット定義は配列（`globalSearch: KeyboardShortcut[]`）だが、実行時には `Proxy(Object) {0: {...}, 1: {...}}` に変換される
- `Array.isArray()` や `Symbol.iterator` が失敗するため、マッチング処理が実行されない

**修正**:
- `matchesAnyShortcut()` 関数で `Object.values()` を使用：
  ```typescript
  const shortcutsArray = Array.isArray(shortcuts) ? shortcuts : Object.values(shortcuts)
  ```
- これにより Proxy(Object) を正しく処理できるように
...
ユーザーが検索モードを理解しやすいよう、説明文を更新：
- 「自動選択」→「自動(カード名+テキスト+Pテキスト)」
...
### デッキ表示ページのカード詳細表示の条件付け

カード詳細表示オプションが無効な場合、Vueアプリケーション全体と公式スタイルの書き換えが適用されなくなりました。

#### 実装内容
1. **コンテナのマウント条件化**:
   - `showCardDetailInDeckDisplay` が有効な場合のみ Vue アプリをマウント
   - 無効な場合は `#ygo-next-deck-display-app` コンテナが作成されない

2. **HTMLクラスの条件付き付与**:
   - `showCardDetailInDeckDisplay` が有効な場合：`<html>` に `ygo-next` と `ygo-next-valid-card-tab-on-deck-display` クラスを付与
   - 無効な場合：両方のクラスを削除

3. **SCSS の条件付きスコープ**:
   - すべてのスタイル書き換えを `html.ygo-next.ygo-next-valid-card-tab-on-deck-display` セレクタで囲む
   - クラスが付与されている場合のみスタイルが適用される

#### メリット
- 不要なDOM要素が生成されない
- 公式サイトのスタイルが予期しない方法で上書きされない
- メモリ使用量の削減
