...
#### 一人回しカードサイズ
一人回し（practice）モードのカードサイズを1P/2Pそれぞれ個別に設定します。
- **1P**: 1人回し時のカードサイズ（デフォルト: M）
- **2P**: 2人対戦時のカードサイズ（デフォルト: S）
...
#### 右エリアの幅
検索結果・カード詳細を表示する右側エリアの幅を設定します。
- **S**: 300px（コンパクト）
- **M**: 400px（デフォルト）
- **L**: 500px
- **XL**: 600px
- **MAX-FIT**: 残りのスペースを埋める（デッキレシピと2:1の比率で分配）

**注意**: MAX-FIT選択時は、デッキレシピ側が優先的にスペースを確保します（最小幅400px）。

#### 右エリアのフォントサイズ
右側エリア内のフォントサイズを設定します。
- **S**: 12px
- **M**: 14px（デフォルト）
- **L**: 16px
- **XL**: 18px
...
#### レベル/ランク/リンクの並び順
デッキソート時のレベル・ランク・リンクの並び順を設定します。

- **降順**（デフォルト）: 高い順に並び替え（Lv12 → Lv1）
- **昇順**: 低い順に並び替え（Lv1 → Lv12）
- **降順/昇順**: 5秒以内に連続してソートを実行すると昇順に切り替わります

**注意**: この設定はデッキ編集画面の「ソート」機能に適用されます。検索結果のソート順には影響しません。

#### カテゴリ優先エリア内の並び順
デッキメタデータで設定したカテゴリに該当するカードの、カテゴリ内での並び順を設定します。

- **レベル順**（デフォルト）: 上記「レベル/ランク/リンクの並び順」設定に従ってソート
- **枚数の降順**: 採用枚数が多いカードを先頭に配置（降順固定）

#### エクスポートファイル名にタイムスタンプを付与
エクスポート時のファイル名に日時（YYYYMMDD-HHmm形式）を付与するかを設定します。

- **有効**（デフォルト）: `デッキ名_20260303-1500.csv` 形式
- **無効**: `デッキ名.csv` 形式
