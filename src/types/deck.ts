...
export interface DeckInfo {
  ...
  /** ソート優先度カテゴリ（複数指定可、フリーテキスト形式）
   * カード名、ルビ、テキスト内に含まれるキーワード/カテゴリを指定
   * 例: ["ライロ", "スプライト", "トライブリガード"]
   */
  sortPriorityCategories?: string[];

  /** 優先度カテゴリ内でのカード枚数による重み付け
   * true: 枚数が多いほど先頭に配置（デッキコアカード優先）
   * false: 優先度カテゴリ内での既存ソート順を維持
   */
