  ...
  setup(props, { emit }) {
    ...
    const sortCards = (cards, sortOrder) => {
      ...
      switch (sortOrder) {
        ...
        case 'atk_desc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (b.atk ?? -1) - (a.atk ?? -1)))
        case 'atk_asc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (a.atk ?? -1) - (b.atk ?? -1)))
        case 'def_desc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (b.def ?? -1) - (a.def ?? -1)))
        case 'def_asc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (a.def ?? -1) - (b.def ?? -1)))
        case 'level_desc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (b.levelValue || 0) - (a.levelValue || 0)))
        case 'level_asc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (a.levelValue || 0) - (b.levelValue || 0)))
        case 'attribute_asc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (a.attribute || '').localeCompare(b.attribute || '')))
        case 'attribute_desc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (b.attribute || '').localeCompare(a.attribute || '')))
        case 'race_asc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (a.race || '').localeCompare(b.race || '')))
        case 'race_desc':
          return sorted.sort(createMonsterPropertySorter((a, b) => (b.race || '').localeCompare(a.race || '')))
        case 'code_asc':
