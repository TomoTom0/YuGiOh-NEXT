...
describe('E2E: Deck Edit → Export → Import Flow', () => {
  ...
  beforeEach(() => {
    ...
    sampleDeck = {
      ...
      mainDeck: [
        ...
        { cid: '4861', ciid: '2', quantity: 2 }
      ],
      extraDeck: [
        { cid: '9753', ciid: '1', quantity: 1 },
        { cid: '6172', ciid: '1', quantity: 2 }
      ],
      sideDeck: [
  ...
  describe('CSV Format', () => {
    it('should successfully export and import deck via CSV', () => {
      ...
      // メインデッキの検証
      const mainCard1 = imported.mainDeck.find(e => e.cid === '12950');
      expect(mainCard1).toBeDefined();
      expect(mainCard1!.quantity).toBe(3);
      expect(mainCard1!.ciid).toBe('1');

      // エクストラデッキの検証
      const extraCard1 = imported.extraDeck.find(e => e.cid === '9753');
      expect(extraCard1).toBeDefined();
      expect(extraCard1!.quantity).toBe(1);

      // サイドデッキの検証
      const sideCard1 = imported.sideDeck.find(e => e.cid === '14558');
  ...
  describe('PNG Format', () => {
    it('should successfully embed and extract deck info from PNG', async () => {
      ...
      expect(mainCard1).toBeDefined();
      expect(mainCard1!.quantity).toBe(3);
      expect(mainCard1!.cid).toBe('12950');
    });
  ...
  describe('Round-trip Tests', () => {
    ...
    it('should maintain card IDs and quantities through TXT round-trip', () => {
      ...
      const mainCard = imported.deckInfo!.mainDeck.find(e => e.cid === '12950');
      expect(mainCard!.quantity).toBe(3);
      
      const extraCard = imported.deckInfo!.extraDeck.find(e => e.cid === '9753');
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle cards with special characters in names', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.deckInfo!.mainDeck.length).toBe(1);
      expect(result.deckInfo!.mainDeck[0].cid).toBe('123');
    });
