...
import { parseForbiddenLimitedHtml, getNextEffectiveDate } from '@/api/forbidden-limited';
...
describe('禁止制限リストAPI', () => {
  describe('parseForbiddenLimitedHtml', () => {
    it('[covers:parse.forbidden_section.present,extract_cards.cid_matched,parse.fetched_at_uses_current_time] サンプルHTMLから禁止制限情報を正しくパースする', () => {
      const mockHtml = `
        <html>
          <body>
    ...
    it('[covers:parse.always_no_throw,parse.fetched_at_uses_current_time] 空のHTMLでも正しく処理する', () => {
      const mockHtml = `
        <html>
          <body>
          </body>
    ...
    it('[covers:parse.forbidden_section.present] 実際のサンプルHTMLファイルをパースできる（存在する場合）', () => {
      try {
  ...
  describe('getNextEffectiveDate', () => {
    it('[covers:next_date.month_lt_4] 1月の日付から次回適用日（4月1日）を計算する', () => {
      const currentDate = new Date('2025-01-15');
      const result = getNextEffectiveDate(currentDate);
      expect(result).toBe('2025-04-01');
    });

    it('[covers:next_date.month_lt_4] 3月の日付から次回適用日（4月1日）を計算する', () => {
      const currentDate = new Date('2025-03-31');
      const result = getNextEffectiveDate(currentDate);
      expect(result).toBe('2025-04-01');
    });

    it('[covers:next_date.month_lt_7] 4月の日付から次回適用日（7月1日）を計算する', () => {
      const currentDate = new Date('2025-04-15');
      const result = getNextEffectiveDate(currentDate);
      expect(result).toBe('2025-07-01');
    });

    it('[covers:next_date.month_lt_10] 7月の日付から次回適用日（10月1日）を計算する', () => {
      const currentDate = new Date('2025-07-15');
      const result = getNextEffectiveDate(currentDate);
      expect(result).toBe('2025-10-01');
    });

    it('[covers:next_date.month_ge_10_wraps_year] 10月の日付から次回適用日（翌年1月1日）を計算する', () => {
      const currentDate = new Date('2025-10-15');
      const result = getNextEffectiveDate(currentDate);
      expect(result).toBe('2026-01-01');
    });

    it('[covers:next_date.month_ge_10_wraps_year] 12月の日付から次回適用日（翌年1月1日）を計算する', () => {
