...
describe('カード検索API', () => {
  ...
  describe('searchCards (name search)', () => {
    it('[covers:search_cards.success_parses_results] カード名で検索し、結果を返す', async () => {
      const mockResponse = `
        <html>
          <body>
            <div id="main980">
    ...
    it('[covers:build_params.card_type_and_logic] ctypeを指定してモンスターカードのみ検索できる', async () => {
    ...
    it('[covers:build_params.card_type_and_logic] ctypeを指定して魔法カードのみ検索できる', async () => {
      const mockResponse = '<html><body></body></html>';
      (global.fetch as Mock).mockResolvedValue({
        ok: true,
        text: async () => mockResponse
    ...
    it('[covers:build_params.card_type_and_logic] ctypeを指定して罠カードのみ検索できる', async () => {
      const mockResponse = '<html><body></body></html>';
      (global.fetch as Mock).mockResolvedValue({
        ok: true,
        text: async () => mockResponse
    ...
    it('[covers:search_cards.success_parses_results,parse_results.missing_containers_empty] 検索結果が0件の場合は空配列を返す', async () => {
      const mockResponse = '<html><body></body></html>';
      (global.fetch as Mock).mockResolvedValue({
        ok: true,
        text: async () => mockResponse
    ...
    it('[covers:search_cards.http_error_empty] fetchが失敗した場合は空配列を返す', async () => {
      (global.fetch as Mock).mockResolvedValue({
        ok: false,
        status: 500
      });
  ...
  describe('searchCardById', () => {
    it('[covers:search_by_id.request_and_first_result] カードIDで検索し、結果を返す', async () => {
      const mockResponse = `
        <html>
          <body>
            <div id="main980">
    ...
    it('[covers:search_by_id.request_and_first_result] 検索結果が0件の場合はnullを返す', async () => {
      const mockResponse = '<html><body></body></html>';
      (global.fetch as Mock).mockResolvedValue({
        ok: true,
        text: async () => mockResponse
    ...
    it('[covers:search_by_id.http_error_null] fetchが失敗した場合はnullを返す', async () => {
      (global.fetch as Mock).mockResolvedValue({
        ok: false,
        status: 500
      });
  ...
  describe('HTML Parsing Edge Cases', () => {
    it('[covers:parse_results.missing_containers_empty] 空のHTMLを正しく処理する', async () => {
      const mockResponse = '<html><body></body></html>';
      (global.fetch as Mock).mockResolvedValue({
        ok: true,
        text: async () => mockResponse
    ...
    it('[covers:parse_results.missing_containers_empty] 不正な形式のHTMLでも空配列を返す', async () => {
    ...
    it('[covers:parse_base.required_name] カード名が欠落している場合でも処理を継続する', async () => {
      const mockResponse = `
        <html>
          <body>
            <div id="main980">
    ...
    it('[covers:parse_base.required_link_value] カードIDが欠落している場合でも処理を継続する', async () => {
      const mockResponse = `
        <html>
          <body>
            <div id="main980">
    ...
    it('[covers:search_cards.catch_empty] 複数のカードを正しくパースする', async () => {
      // 実際のHTML構造に基づいたテスト
      const result = await searchCards({
        keyword: 'test',
        searchType: '1',
    ...
    it('[covers:search_cards.catch_empty] ネットワークエラー時に空配列を返す', async () => {
    ...
    it('[covers:search_cards.catch_empty] fetchの例外時に空配列を返す', async () => {
  ...
  describe('parseLinkValue', () => {
    it('[covers:parse_link.valid_directions_set_bits] 単一の方向をビット値に変換する', () => {
    ...
    it('[covers:parse_link.invalid_and_center_ignored] 方向5（中央）は無視される', () => {
    ...
    it('[covers:parse_link.valid_directions_set_bits] 複数の方向を正しく組み合わせる', () => {
      // 例: "13" → 方向1（左下）と3（右下）
    ...
    it('[covers:parse_link.valid_directions_set_bits] 実際のリンクモンスター例: "123456789"（全方向）', () => {
      // 方向5（中央）を除く全てのビットが立つ
      expect(parseLinkValue('123456789')).toBe(0b111101111); // = 495
    });

    it('[covers:parse_link.invalid_and_center_ignored] 範囲外の値は無視される', () => {
    ...
    it('[covers:parse_link.invalid_and_center_ignored] 空文字列は0を返す', () => {
      expect(parseLinkValue('')).toBe(0);
    });

    it('[covers:parse_link.valid_directions_set_bits] 同じ方向が複数回指定されても冪等性がある', () => {
  ...
  describe('parseSearchResults', () => {
    it('[covers:parse_results.rows_push_only_truthy] DOM DocumentからCardInfo配列をパースする', () => {
      const mockHtml = `
        <html>
          <body>
            <div id="main980">
    ...
    it('[covers:parse_results.missing_containers_empty] カードが0件の場合は空配列を返す', () => {
  ...
  describe('extractImageInfo', () => {
    it('[covers:extract_image.regex_ciids_and_enc] 画像情報を抽出してMapを返す', () => {
      const mockHtml = `
        <html>
          <body>
            <div id="card_list">
  ...
  describe('parseCardBase', () => {
    it('[covers:parse_base_optional_fields_and_image_fallback] HTML要素からCardBase情報をパースする', () => {
      const mockHtml = `
        <div class="t_row c_normal">
    ...
    it('[covers:parse_base.required_link_value] 必須要素が欠落している場合はnullを返す', () => {
      const mockHtml = `
        <div class="t_row c_normal">
          <!-- link_value がない -->
          <div class="box_card_name">
  ...
  describe('buildSearchParams', () => {
    it('[covers:build_params.base_defaults] 基本的なキーワード検索パラメータを構築する', () => {
    ...
    it('[covers:build_params.card_type_and_logic] カードタイプフィルタを正しく設定する', () => {
    ...
    it('[covers:build_params.monster_filters_append_each] 属性フィルタを正しく設定する', () => {
    ...
    it('[covers:build_params.atk_def_zero_falls_back_empty] 攻撃力範囲を正しく設定する', () => {
      const params = buildSearchParams({
        keyword: '',
    ...
    it('[covers:build_params.atk_def_zero_falls_back_empty] 守備力範囲を正しく設定する', () => {
      const params = buildSearchParams({
        keyword: '',
    ...
    it('[covers:build_params.level_scale_link_range] レベルフィルタを正しく設定する', () => {
      const params = buildSearchParams({
        keyword: '',
        levels: [4, 7, 8]
      });
    ...
    it('[covers:build_params.level_scale_link_range] リンク数フィルタを正しく設定する', () => {
      const params = buildSearchParams({
        keyword: '',
        linkNumbers: [1, 2, 3]
      });
    ...
    it('[covers:build_params.level_scale_link_range] リンクマーカー方向を正しく設定する', () => {
      const params = buildSearchParams({
        keyword: '',
    ...
    it('[covers:build_params.card_type_and_logic] リンクマーカー論理演算を正しく設定する', () => {
    ...
    it('[covers:build_params.base_defaults] ソート順を正しく設定する', () => {
    ...
    it('[covers:build_params.base_defaults] 結果件数を正しく設定する', () => {
      const defaultParams = buildSearchParams({ keyword: '' });
    ...
    it('[covers:build_params.release_date_partial] 発売日範囲を正しく設定する', () => {
      const params = buildSearchParams({
        keyword: '',
        releaseDate: {
  ...
  describe('getCardDetailWithCache', () => {
    ...
    it('[covers:get_cache.cache_miss_fetch_and_save] キャッシュがない場合はAPIを呼び出す', async () => {
      // getCardDetail をモック
    ...
    it('[covers:get_detail.base_from_unified_then_temp,get_cache.cache_miss_fetch_and_save] 無効なカードIDの場合はdetailがnullになる', async () => {
      global.fetch = vi.fn().mockResolvedValue({
        ok: true,
    ...
    it('[covers:get_detail.catch_null,get_cache.cache_miss_fetch_and_save] fetchエラー時はdetailがnullになる', async () => {
  ...
  describe('parseMonsterCard', () => {
    it('[covers:parse_monster.attribute_required] 属性が欠落している場合はnullを返す', () => {
      const mockHtml = `
        <div class="t_row c_normal">
  ...
  describe('parseSpellCard', () => {
    it('[covers:parse_spell.effect_or_normal] 魔法カードを正しくパースする', () => {
      const mockHtml = `
        <div class="t_row c_normal">
          <div class="box_card_attribute">
            <img src="/yugiohdb/icon/attribute_icon_spell.png">
    ...
    it('[covers:parse_spell.attribute_required] 魔法でない場合はnullを返す', () => {
      const mockHtml = `
        <div class="t_row c_normal">
          <div class="box_card_attribute">
  ...
  describe('parseTrapCard', () => {
    it('[covers:parse_trap.effect_or_normal] 罠カードを正しくパースする', () => {
      const mockHtml = `
        <div class="t_row c_normal">
          <div class="box_card_attribute">
            <img src="/yugiohdb/icon/attribute_icon_trap.png">
    ...
    it('[covers:parse_trap.attribute_required] 罠でない場合はnullを返す', () => {
      const mockHtml = `
        <div class="t_row c_normal">
          <div class="box_card_attribute">
            <img src="/yugiohdb/icon/attribute_icon_spell.png">
