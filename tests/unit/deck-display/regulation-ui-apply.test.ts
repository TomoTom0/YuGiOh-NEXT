 ...
 *   4. GENESYS解決順序（TASK-472）: ensureParsedDeckInfo()（そのデッキ自身のカードをカードDBへ
 *      登録する）がGENESYS解決（ensureResolvedList内のensureList/ensureCurrentList）より先に
 *      呼ばれること。OCG/タグ無しモードではensureParsedDeckInfo()自体を呼ばないこと
 *
...
import { setupRegulationDisplay } from '@/content/deck-display/regulation-ui';
...
describe('setupRegulationDisplay の適用パス', () => {
  ...
  it('[covers:regulation_apply.genesys_ensures_parsed_deck_info_before_list_resolution] GENESYSモード適用時、ensureParsedDeckInfo()がGENESYS解決（ensureResolvedList）より先に呼ばれる', async () => {
    mocks.deckName.value = '[GENESYS] テストデッキ';
    mocks.genesys.ensureCurrentList.mockResolvedValue({ list: true });

    await setupRegulationDisplay(mocks.ensureParsedDeckInfo);

    expect(mocks.ensureParsedDeckInfo).toHaveBeenCalledTimes(1);
    expect(mocks.genesys.ensureCurrentList).toHaveBeenCalledTimes(1);
    const parseOrder = mocks.ensureParsedDeckInfo.mock.invocationCallOrder[0];
    const resolveOrder = mocks.genesys.ensureCurrentList.mock.invocationCallOrder[0];
    expect(parseOrder).toBeLessThan(resolveOrder);
  });

  it('[covers:regulation_apply.non_genesys_modes_skip_ensure_parsed_deck_info] OCG過去版適用時、ensureParsedDeckInfo()は呼ばれない（不要なデッキパースを避ける・既存挙動の維持）', async () => {
    mocks.deckName.value = '[OCG-2401] テストデッキ';
    mocks.ocg.getRegulation.mockReturnValue('limited');

    await setupRegulationDisplay(mocks.ensureParsedDeckInfo);

    expect(mocks.ensureParsedDeckInfo).not.toHaveBeenCalled();
  });

  it('[covers:regulation_apply.non_genesys_modes_skip_ensure_parsed_deck_info] タグ無しデッキ（mode=none）適用時、ensureParsedDeckInfo()は呼ばれない', async () => {
    await setupRegulationDisplay(mocks.ensureParsedDeckInfo);
