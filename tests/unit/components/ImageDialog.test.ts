...
describe('ImageDialog.vue - refreshPreview', () => {
  ...
  it('TASK-390: プレビューの縁取りはCSS borderで表現され、ダウンロード画像には焼き込まれない（selectedColorのaccentLineに連動）', async () => {
    wrapper = await mountInitializedDialog();

    // デフォルト(red)の時点でプレビュー要素にaccentLine色のborderColorが設定されている
    let style = wrapper.find('.background-image').attributes('style') ?? '';
    expect(style).toContain('border-color: #ed1b1b');

    // 色を変更するとborderColorも追従する（画像生成オプションのcolorはCanvas描画には使うが、
    ...
    expect(style).toContain('border-color: #1485ed');
  });
