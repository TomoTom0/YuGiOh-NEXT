...
### 現在対応中のバグ・課題
1. ⚠️ **Deploy未実施のため、最新のビルド結果が反映されていない**
   - ビルドは成功しているが、手動でのデプロイが必要
   
2. ⏳ バグ修正（優先度：高）
   - 検索結果でカード画像が表示されない（画像URL生成関数に`lang=1`パラメータが含まれる問題）
   - Card tabのタブ切り替えが動作しない
   - ホバー時の「ME」表示を「M」に変更
   
3. ⏳ スタイル調整
   - Right areaの幅を320pxに統一
   - セクションの高さ調整（Main: 2行、Extra/Side: 2行、Trash: 1行）
   - カード画像の左詰め配置
   - グリッド表示時の余白削除
